...
transaction {
    prepare(signer: AuthAccount) {
        ...
        if let ref = signer.borrow<&ChildAccount.ChildAccountCreator>(from: ChildAccount.ChildAccountCreatorStoragePath) {
            return
        }
        signer.save(<-ChildAccount.createChildAccountCreator(), to: ChildAccount.ChildAccountCreatorStoragePath)
        // Link the public Capability
        signer.link<&{ChildAccount.ChildAccountCreatorPublic}>(
